package me.reklessmitch.mitchprisonscore.mitchgenerator;
    ...
    public CleanroomChunkGenerator(String id) {
        ...
        try {
            ...
            for (int i = 0; i < layerCount; i++) {
                ...
                if (height <= 0) {
                    log.warning("[MPC] Invalid height '" + tokens[j] + "'. Using 64 instead.");
                    height = 64;
                }
                ...
                try {
                    ...
                    log.warning("[MPC] Failed to lookup block '" + tokens[j + 1] + "'. Using stone instead. Exception: " +
                            e.toString());
            ...
            log.severe("[MPC] Error parsing CleanroomGenerator ID '" + id + "'. using defaults '64,1': " + e.toString());
            e.printStackTrace();
