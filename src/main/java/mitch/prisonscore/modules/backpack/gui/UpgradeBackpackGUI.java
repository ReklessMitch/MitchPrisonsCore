...
public class UpgradeBackpackGUI extends ChestGui {
    ...
    private void getUpgradeItem(int[] slots, BigInteger amount, BigInteger cost){
        ...
        final TagResolver costResolver = Placeholder.parsed("cost", CurrencyUtils.format(cost));
        final TagResolver amountResolver = Placeholder.parsed(
                "amount",
                amount.compareTo(BigInteger.valueOf(1000)) > 0 ? CurrencyUtils.format(amount) : amount.toString()
        );

        final ItemStack item = BackpackModule.get().getBackpackUpgradeItem().getFormatItem(costResolver, amountResolver);
        for(int slot : slots) {
    ...
    public void refresh(){
        ...
        getUpgradeItem(new int[]{3,4,5,12,13,14}, BigInteger.valueOf(10), backpackPlayer.getCost(BigInteger.valueOf(10)));
        ...
        final BigInteger maxPurchasable = backpackPlayer.getMaxPurchasable();
    ...
    private void getAutoSellItem() {
        ...
        final LangConf conf = LangConf.get();
        long cost = BackpackModule.get().getAutoSellCost();
